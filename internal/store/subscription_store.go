...
func (s *MemoryStore) ListSubscriptionsByEndpointID(endpointID string) []*model.Subscription {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *MemoryStore) ListSubscriptionsByEventType(eventType string) []*model.Subscription {
	s.mu.RLock()
	defer s.mu.RUnlock()
	list := make([]*model.Subscription, 0)
	for _, sub := range s.subscriptions {
